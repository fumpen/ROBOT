...
class FrindosInnerWorld:
    ...
    def update_from_update_particle(self, dicte):
        print '### update_from_update_particle ###'
        self.update_particles(dicte['particles'])
        self.update_l_flag(dicte['obs_obj'][3])
        print "update_from_update_particle obs_obj[3]: "  + str(dicte['obs_obj'][3])
...
def turn(dir, deg, inner_frindo):
    ...
    if dir == 'left':
        ret_dict = p.update_particles(inner_frindo.getParticles(), cam, 0.0,
                                      deg-5, world, WIN_RF1, WIN_World)
    else:
        ret_dict = p.update_particles(inner_frindo.getParticles(), cam, 0.0,
                                    ((-1.0) * (deg-5)), world, WIN_RF1, WIN_World)
...
def angle_correction(inner_frindo, goal_number):
    ...
    ret_dict = turn(ret_landmark['dir'], np.degrees(ret_landmark['deg']),
                    inner_frindo)
    print 'ret_landmark ang: ', np.degrees(ret_landmark['deg'])
    print 'ret_dict ang: ', np.degrees(ret_dict['obs_obj'][2])
    inner_frindo.update_from_update_particle(ret_dict)
    ...
    while -10.0 > deg_to_turn or 10.0 < deg_to_turn:
        ...
        if ret_landmark['dir'] is not None:
            ...
            return False
        print '___angle_correction degree___: ' + str(deg_to_turn)
    return True
